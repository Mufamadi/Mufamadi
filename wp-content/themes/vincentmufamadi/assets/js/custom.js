...
/*-----------------------------------------------------------------------------------*/
    ...
    function blogSingle() {
        var $container = $('.single .touch-slider');
...
/*-----------------------------------------------------------------------------------*/	 
	...
	function Nav(){
		...
		$('.go-to-top').click(function(e){
			...
			$('.active-menu').removeClass('active-menu');
			$('a[href*="home"]').addClass('active-menu');
...
/*-----------------------------------------------------------------------------------*/	 
   ...
   function Scroll(){
		...
		if ( ($(window).width() >=768) ) {
			...
			$('.vertical-page,.single ,.blog ,.archive , .search-results , .page-template-template-page-php , .page-template-default').niceScroll({autohidemode:true,cursorwidth:"8px"});
		}
	}   
    ...
    function Ajax_Load_PPage() {
        ...
        $btn.click(function () {
            ...
            $pageContainer.load(portfolio_data.nextLink + ' .portfolio-post', function () {
				...
				$cycle= $(".portfolio-post").find('#cycle-slider');
				$cycle.cycle();
...
/*-----------------------------------------------------------------------------------*/
/*	Return measure that page should scroll
/*-----------------------------------------------------------------------------------*/

    function liNum ($this){
        var elemClass= $this.parents('ul').attr('class');
        if((elemClass != null) && (elemClass.indexOf('mobile-ul')>=0))
            var activeLi=$('.mobile-ul li').eq(0).children('a').attr('class');
        else
          var  activeLi=$('.menu ul li').eq(0).children('a').attr('class');
        var i= 0,sum= 0;
        if(ifPageExist($this.attr('href')))
            while(activeLi != "active-menu"){
                if(elemClass.indexOf('mobile-ul')>=0)
                    var tempLi=$('.mobile-ul li').eq(i);
                else
                    var  tempLi=$('.menu ul li').eq(i);
                var pageName=tempLi.children('a').attr('href');
                if(ifPageExist(pageName))
                    if($(window).width() <=768)
                        sum+=$('#'+pageName).outerHeight();
                    else
                        sum+=$('#'+pageName).outerWidth();
                    i++;
                    if(elemClass.indexOf('mobile-ul')>=0)
                        var activeLi=$('.mobile-ul li').eq(i).children('a').attr('class');
                    else
                        var  activeLi=$('.menu ul li').eq(i).children('a').attr('class');
            }
             return sum;
    }
